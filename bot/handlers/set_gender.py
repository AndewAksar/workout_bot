...
async def set_gender(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if mode == "api":
        ...
        return await _reply_and_finish(
            ...
            (
                ...
                "Проверьте раздел «👤 Показать профиль», чтобы убедиться, что"
                " данные обновились. Эти сведения помогают формировать"
    ...
    return await _reply_and_finish(
        ...
        (
            "✅ Пол успешно обновлён!\n"
            "Загляните в «👤 Показать профиль», чтобы увидеть актуальные"
            " данные. При необходимости можно изменить другие показатели"
            " через меню ниже."
        ),
        reply_markup=get_personal_data_menu(),
        delete_user_message=True,
